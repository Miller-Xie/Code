* **二叉树总结**
  ...
  * **前序遍历**
    ...
    * [二叉树最大宽度](#二叉树最大宽度) (`medium` `dfs` `bfs`)
  * **中序遍历**
...
[LeetCode中文](https://leetcode.cn/problems/maximum-width-of-binary-tree/description/)

题解：https://leetcode.cn/problems/maximum-width-of-binary-tree/solutions/1779284/by-muse-77-7hwf/

bfs和dfs两种解法，需要优化数值过大超出范围的问题
...
class Solution {
public:
    void dfs(TreeNode* p, int level, long long idx, int& res) {
        if (mp.find(level) == mp.end()) {
            mp[level] = idx;
        }
        res = max(res, int(idx - mp[level] + 1));
        idx = idx - mp[level] + 1;
        if (p->left) {
            dfs(p->left, level + 1, idx * 2, res);
        }
        if (p->right) {
            dfs(p->right, level + 1, idx * 2 + 1, res);
        }
    }
