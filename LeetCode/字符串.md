...
假设第n项和第n-1项的递推关系为 *a<sup>n</sup> = f(a<sup>n-1</sup>)*,定义一个函数`getStr`来代替函数*f*，初始化数列首项"1"和"11"，根据函数`getStr`推导出第n项即可。
...
class Solution {
public:
    bool is_ok(int idx1,int idx2,string str1,string str2)
    {
