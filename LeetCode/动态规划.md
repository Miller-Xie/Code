...
**解答**：
方法1：动态规划，dp[i]表示已数字nums[i]结尾的连续子数组的最大子序和，求出dp数组中的**最大值**就是整个数组的最大子序和，空间复杂度O(n)
...
没遍历一个元素nums[i],都将sum和Max比较，负值Max为两者较大值，即Max = max(Max,sum)，最终Max就是整个数组的最大子序和。此方法不需要开辟数组dp，只需要用两个变量sum和Max，空间复杂度为O(1)
...
**解答**：参考链接 [https://www.cnblogs.com/grandyang/p/4383632.html](https://www.cnblogs.com/grandyang/p/4383632.html)
...
**解答**:
方法：**动态规划**，dp[i][j]表示从底部到点triangle[i][j]走过的最短路径长度自下而上取相邻的较小值相加

动态转移方程为：dp[i][j] = triangle[i][j] + min(dp[i+1][j],dp[i+1][j+1]);

空间优化：自下向上计算的时候，可以重复利用大小为n的数组，因为当计算出dp[i][j]之后，dp[i+1][j]就不用了，所以重复计算公式为
