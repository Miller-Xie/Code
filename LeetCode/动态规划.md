...
方法2：分治 + 贪心  
找出交易一次能获得最大利润的区间范围[b,e]（参考[买卖股票的最佳时机II]()），设最大利润为max_all，再找出这个区间内交易一次亏损最多的区间[bmid,emid]，设最大亏损为max_mid < 0。同样的方法，找出左边区间[0,b-1]内交易一次能获得的最大利润max_left和右边区间[e+1,len-1]内交易一次能获得的的最大利润max_right。找出三者的最大值max(max_left,max_right,-max_mid)，那么买卖两次股票能获得最大利润有可能是以下三种情况：
